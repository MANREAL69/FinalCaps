<div>
    {{ $slot ?? '' }} <!-- This will output the slot content if it exists -->
    <div
       ...
       class="flex flex-col transition-all h-full overflow-hidden">
        ...
        <main class="overflow-y-scroll overflow-hidden grow h-full relative" style="contain:content">
            {{-- chatlist --}}
            <ul class="p-2 grid w-full space-y-2" aria-live="polite">
                @if ($conversations)
                    @foreach ($conversations as $key => $conversation)
                        <li id="conversation-{{$conversation->id}}" wire:key="{{$conversation->id}}" 
                            ...
                            role="listitem">
                            <a href="#" class="shrink-0" aria-label="Chat with {{ $conversation->getReceiver()->name }}">
                            ...
                            <aside class="grid grid-cols-12 w-full">
                                ...
                                <div class="col-span-1 flex flex-col text-center my-auto">
                                    <x-dropdown align="right" width="48">
                                        <x-slot name="trigger">
                                        <button aria-label="View Profile of {{ $conversation->getReceiver()->name }}">
                    ...
                    @endforeach
                @else
                    <li role="alert">No conversations available.</li>
                @endif
            </ul>
        </main>
    </div>
